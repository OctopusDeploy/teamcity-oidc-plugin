...
- [ ] **Private keys are only obfuscated, not encrypted** — `EncryptUtil.scramble` is a fixed-key
      XOR; anyone with read access to the plugin data directory gets the full private signing keys.
      The POSIX 0600 permissions help but any process running as the TC service account is
      unprotected. Options: integrate with a KMS (Vault, AWS KMS, GCP KMS), store in a
      password-protected PKCS12 KeyStore, or at minimum document the threat model clearly.

### High

- [ ] **SSRF in `exchange` test step** — `serviceUrl` is accepted from the HTTP request and used
      to issue an outbound HTTP GET with no scheme check, no allowlist, and no IMDS protection.
      Reachable destinations include `http://169.254.169.254/` and any internal RFC-1918 host.
      Fix: require HTTPS, validate host against an allowlist or at minimum block link-local/loopback
      addresses, or remove the exchange step from the server side entirely.
...
- [ ] **`exchange` step exfiltrates a live signed JWT to an arbitrary URL** — the step POSTs a
      real, unexpired JWT to the attacker-supplied `serviceUrl`. When `audience` is blank there is
      no audience restriction. Fix: validate `serviceUrl` (see SSRF finding) and require a
      non-empty audience.
- [ ] **Algorithm parameter is unconstrained** — `JwtKeyManager.sign()` accepts any string and
...
- [ ] **No upper bound on JWT TTL** — `PropertiesProcessor` only requires a positive integer; a
...
- [ ] **Token replay is possible until expiry** — `jti` is generated but never tracked server-side.
      A captured token can be replayed freely. Deferred per the existing "no revocation list" policy,
      but worth re-evaluating given short TTLs are not enforced (see above).
- [ ] **`triggered_by` claim leaks internal TC descriptor strings** — `triggeredBy.getAsString()`
      is an unstable internal format that has historically included user display names, schedule
      expressions, and VCS rule descriptions. Consider omitting or sanitising this claim.
- [ ] **`serviceUrl` accepts HTTP; live JWT transmitted in plaintext** — no scheme restriction
      combined with the credential exfiltration risk above. Fix: require HTTPS for `serviceUrl`.
- [ ] **`stepDiscovery` and `stepJwks` fetch without checking scheme** — `isHttpsUrl()` is not
      called before the internal HTTP fetches in the test steps.

### Low

- [ ] **Exception messages returned verbatim to the client** — `e.getMessage()` in
      `JwtTestController` and `KeyRotationController` may expose file paths or internal topology.
      Fix: log the full exception server-side; return a generic message to the client.
- [ ] **JWKS JSP guard is at page level, not JSP level** — `CHANGE_SERVER_SETTINGS` is checked
      in `JwtBuildFeatureAdminPage.isAvailable()` but not in the JSP itself.
- [ ] **Post-rotation JWKS cache lag** — `Cache-Control: max-age=300` means intermediate caches
...
- [ ] **No automatic key rotation schedule** — keys are rotated only manually; indefinite key reuse
      increases blast radius of a compromise. (See existing "Key rotation schedule" item above.)
- [ ] **Only one generation of retired keys preserved** — tokens spanning two rotations fail
      verification. Acceptable given short TTLs; document this limitation.
- [ ] **`triggered_by_id` is a numeric TC user ID** — may be reused if a user is deleted and
      recreated; not a stable identity across user lifecycle events.
- [ ] **Discovery document omits `authorization_endpoint`** — may cause strict OIDC validators to
      reject the document; this provider is non-interactive so the field is not meaningful, but
      some validators require it.
- [ ] **RSA key size is 2048 bits** — acceptable per NIST until 2030 but below the 3072+ now
      recommended for new long-lived signing keys. Low priority given the manual rotation path.
- [ ] **Test-issued JWTs omit `nbf`** — production tokens include `nbf`; test tokens don't.
