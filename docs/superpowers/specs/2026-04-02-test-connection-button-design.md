...
- Optionally: POST the JWT to a user-supplied exchange URL (entered at test time, not saved)
...
| `exchange` | HTTP POST to `exchangeUrl` with `Content-Type: application/x-www-form-urlencoded` body: `grant_type=urn:ietf:params:oauth:grant-type:token-exchange&subject_token={token}&subject_token_type=urn:ietf:params:oauth:token-type:jwt`. Reports HTTP status and first 200 chars of response body. | `Exchange succeeded (HTTP 200)` |
...
- An exchange section below a divider: a text input for the exchange URL and a "Try Exchange" button (disabled until step 3 passes)
...
6. If user enters a URL and clicks "Try Exchange": POST `step=exchange` with `token` + `exchangeUrl` → update row 4
...
The exchange URL and the test JWT are transient — they exist only in the browser session during the test. Nothing is written to disk or the TC database.
...
- `step=exchange` with a mock server returning 200 returns `ok: true`
- `step=exchange` with a mock server returning 401 returns `ok: false` with status in message
